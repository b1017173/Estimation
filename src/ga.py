...
class GaSolver(metaclass = ABCMeta):
    ...
    def __init__(self, chromosome_length, population_size, pick_out_size,
                 individual_mutation_probability=0.1, gene_mutation_probability=0.1, iteration=1, verbose=True):        
    ...
    def _uniform_crossover(self, individual1, individual2, chromosome_length):
        """一様交叉関数
        ...
        # 入れ替える点のIndexを算出
        cross_indexs = random.sample(range(chromosome_length), k = chromosome_length//2)

        # 遺伝子を取り出します
        one = individual1.chromosome
        second = individual2.chromosome

        for cross_index in cross_indexs:
            one[cross_index], second[cross_index] = second[cross_index], one[cross_index]
        
        # 子孫
        offsprings = [Individual(one, 0), Individual(second, 0)]
        return offsprings
    ...
    def solve(self, X, y):
        ...
        for count in range(self.iteration):
            ...
            for i in range(0, self.pick_out_size-1):
                offsprings.extend(self._uniform_crossover(elites[i], elites[i+1], self.chromosome_length))
